...
fun String.withFractionsToDouble(separator: Char = com.herohiman.tournant.separator): Double? {
	return try {
		 when {
			 this.contains(" ") -> {
...
fun String.extractFractionsToDouble(separator: Char = com.herohiman.tournant.separator): Pair<Double?, String?> {
	val fraction =
		if (get(0).isDigit()) {
